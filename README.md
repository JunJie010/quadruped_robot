...
- **WiFi 远程控制**  
  基于 ESP8266 模块，采用 **AP 模式** 创建局域网，实现与上位机的无线连接；通过自定义 TCP 透传协议，可远程控制机器人执行前进、转向、停止、扭动、俯卧撑等多种动作。

- **多舵机协同运动**  
  8 个 SG90 舵机分别驱动四个夹爪的水平和垂直运动，通过 **定时器 PWM 输出** 实现精确角度控制；设计 **舵机电源控制电路** 并编写 ADC 驱动实时监测电池电压，确保电压低于 5V 时自动切断舵机电源，避免异常动作。

- **语音实时反馈**  
  集成 JQ8900-16P 语音播报模块，采用 **两线串口指定路径** 播放预存音频。每个动作执行前后自动播放对应提示音（如“左转”“前进”“扭动”等），音量可调，提升人机交互体验。

- **12 种预设动作**  
  编写了完整的动作函数库，包括：**初始复位、四足立起、跳跃、俯卧撑、打招呼、扭动、蠕动、左转、右转、前进** 等，动作流畅，响应迅速。

- **电池电量监测**  
  通过 ADC 实时采集电源电压，动态监测电池状态，防止舵机在欠压情况下误动作，保障系统稳定运行。
...
### 舵机控制电路
![舵机驱动](制作过程/舵机电源控制电路.png)
![舵机驱动](制作过程/舵机电路.png)
![舵机驱动](制作过程/舵机编号及对应引脚.png)
